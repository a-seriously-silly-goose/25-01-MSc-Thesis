...
costs = np.zeros(
    (Nsimulations, envParams["Ndt"], len(rm_list))
)  # matrix to store all testing trajectories
finalprice = np.zeros(
    (Nsimulations, len(rm_list))
)  # matrix to store all testing trajectories
...
for idx_method, method in enumerate(rm_list):
    ...
    actor_critic.policy.load_state_dict(
        T.load(
            repo + "/Pi " + hyperparameters_version + ".pt",
            map_location=T.device("cpu"),
            weights_only=True,
        )
    )
    actor_critic.V.load_state_dict(
        T.load(
            repo + "/V " + hyperparameters_version + ".pt",
            map_location=T.device("cpu"),
            weights_only=True,
        )
    )
    ...
    for timestep in env.spaces["t_space"][:-1]:
        ...
        u, _ = actor_critic.select_actions(
            S, v, alpha, B, timestep * T.ones(Nsimulations), "best"
        )
...
for idx_method, method in enumerate(rm_list):
    ...
    plt.plot(grid, kde(grid), color=utils.colors[idx_method], linewidth=1.5)
    # plot quantiles of the distributions
    plt.axvline(
        x=np.quantile(rewards_total[:, idx_method], 0.1),
        linestyle="dashed",
        color=utils.colors[idx_method],
        linewidth=1.0,
    )
    ...
    #             linewidth=1.0)
    plt.axvline(
        x=np.quantile(rewards_total[:, idx_method], 0.9),
        linestyle="dashed",
        color=utils.colors[idx_method],
        linewidth=1.0,
    )
...
for idx_method, method in enumerate(rm_list):
    plt.scatter(
        finalprice[:, idx_method],
        -rewards_total[:, idx_method]
        - np.maximum(env.params["K"] - finalprice[:, idx_method], 0),
        alpha=0.15,
        s=2,
        color=utils.mred,
    )
...
for idx_method, method in enumerate(rm_list):
    plt.hist(
        (
            rewards_total[:, idx_method]
            - 0 * np.maximum(env.params["K"] - finalprice[:, idx_method], 0)
        )
        ** 2,
        alpha=0.15,
        color=utils.mred,
