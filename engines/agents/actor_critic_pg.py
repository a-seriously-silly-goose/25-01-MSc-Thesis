...
class ActorCriticPG:
    ...
    def select_actions(
        ...
        obs_t = T.stack((S_t.clone(), alpha_t.clone(), time_t.clone()), -1)
        ...
        # get actions
        u_t = T.maximum(
            T.ones(1) * -self.env.params["max_alpha"],
            T.minimum(
                T.ones(1) * self.env.params["max_alpha"], actions_sample.squeeze(-1)
            ),
        )
    ...
    def sim_trajectories(
        ...
        S = T.zeros(
            (Ntrajectories, self.env.params["Ndt"]),
            dtype=T.float,
            requires_grad=False,
            device=self.device,
        )
        ...
        S_tp1 = T.zeros(
            (Ntrajectories, Mtransitions, self.env.params["Ndt"]),
            dtype=T.float,
            requires_grad=False,
            device=self.device,
        )
    ...
    def estimate_V(
        ...
        trajs, transitions = self.sim_trajectories(
            Ntrajectories, Mtransitions, choose="random", seed=rng_seed
        )
        ...
        for epoch in range(Nepochs):
            ...
            obs_t = T.stack((S_batch, alpha_batch, time_batch), -1).detach()
            ...
            obs_tp1 = T.stack(
                (
                    transitions["S_tp1"][batch_idx, :, 1:-1],
            ...
            v_target[:, -1] = self.env.get_final_cost(
                S_batch[:, -1], v_batch[:, -1], alpha_batch[:, -1], B_batch[:, -1]
            )
            ...
            if epoch % self.loss_print == 0 or epoch == Nepochs - 1:
                mean_loss = np.round(
                    np.mean(self.loss_history_V[-self.loss_trail :]), 4
                )

                log_message = f" Epoch = {str(epoch)}, " f" Loss: {str(mean_loss)}"
                print(log_message)
                with open(self.LOG_FILE, "a") as file:
                    file.write(log_message + "\n")
    ...
    def update_policy(
        ...
        for epoch in range(Nepochs):
            ...
            trajs, transitions = self.sim_trajectories(
                batch_size, Mtransitions, choose="random", seed=rng_seed
            )

            # value function of the next time step
            obs_tp1 = T.stack(
                (
                    transitions["S_tp1"][:, :, :-1].clone(),
                    transitions["alpha_tp1"][:, :, :-1].clone(),
                    transitions["timestep_tp1"][:, :, :-1].clone(),
                ),
                -1,
            )
            V_tp1 = self.V(obs_tp1.clone()).squeeze()
            ...
            V_loss = self.risk_measure.get_V_loss(
                transitions["cost_t"][:, :, :-1].detach() + V_tp1.detach(),
            ...
            if epoch % self.loss_print == 0 or epoch == Nepochs - 1:
                ...
                log_message = f"   Epoch =  {str(epoch)} , " f" Loss: {mean_loss}"
                print(log_message)
                with open(self.LOG_FILE, "a") as file:
                    file.write(log_message + "\n")
    ...
    def plot_current_policy(self):
        ...
        hist2dim_pi = np.zeros(
            [len(self.env.spaces["S_space"]), len(self.env.spaces["S_space"]) - 1]
        )
        for S_idx, S_val in enumerate(self.env.spaces["S_space"]):
            for time_idx, time_val in enumerate(self.env.spaces["t_space"][:-1]):
                (
                    hist2dim_pi[len(self.env.spaces["S_space"]) - S_idx - 1, time_idx],
                    _,
                ) = self.select_actions(
                    T.Tensor([S_val]).to(self.device),
        ...
        plt.imshow(
            ...
            cmap=utils.cmap,
            extent=[
                np.min(self.env.spaces["t_space"]),
    ...
    def plot_current_V(self):
        ...
        hist2dim_V = np.zeros(
            [len(self.env.spaces["S_space"]), len(self.env.spaces["t_space"]) - 1]
        )
        for S_idx, S_val in enumerate(self.env.spaces["S_space"]):
            for time_idx, time_val in enumerate(self.env.spaces["t_space"][:-1]):
                obs = T.stack(
                    (S_val * T.ones(1), fixed_alpha * T.ones(1), time_val * T.ones(1)),
                    -1,
                )
                hist2dim_V[len(self.env.spaces["S_space"]) - S_idx - 1, time_idx] = (
                    self.V(obs)
                )
