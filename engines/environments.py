...
class BlackScholesEnv(BaseEnv):
    ...
    def step(self, S_t, v_t, alpha_tm1, B_t, alpha_t):
        ...
        S_tp1 = S_t * T.exp(
            ...
            + self.params.sigma * self.params.sqrt_dt * T.randn(Nsims, device=self.device)
        )
...
class HestonEnv(BaseEnv):
    ...
    def random_reset(self, time, Nsims=1):
        ...
        S0 = self.params.S0 * T.exp(
            ...
            + T.sqrt(T.tensor(self.params.v0 * time, device=self.device)) * T.randn(Nsims, device=self.device)
        )
    ...
    def step(self, S_t, v_t, alpha_tm1, B_t, alpha_t):
        ...
        W2 = self.params.rho * W1 + T.sqrt(T.tensor(1 - self.params.rho ** 2, device=self.device)) * W2

        # Volatility process (CIR)
        v_tp1 = v_t + self.params.kappa * (self.params.theta - T.maximum(v_t, T.zeros(1, device=self.device))) * self.params.dt \
                + self.params.eta * T.sqrt(T.maximum(v_t, T.zeros(1, device=self.device)) * T.tensor(self.params.dt, device=self.device)) * W2

        # Asset price process
        S_tp1 = S_t * T.exp((self.params.r - 0.5 * T.maximum(v_t, T.zeros(1, device=self.device))) * self.params.dt
                            + T.sqrt(T.maximum(v_t, T.zeros(1, device=self.device)) * T.tensor(self.params.dt, device=self.device)) * W1)
